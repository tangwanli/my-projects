...
class HourGlassProgress {
    ...
    setProgressColor(){
        if (this.option.bgColor){
            this.container.querySelector('.hourglass-top').style.borderTopColor = this.option.bgColor;
            this.container.querySelector('.hourglass-bottom-sand').style.borderBottomColor = this.option.bgColor;
        }
        if (this.option.progressColor){
            this.container.querySelector('.hourglass-top-sand').style.borderTopColor = this.option.progressColor;
            this.container.querySelector('.hourglass-bottom').style.borderBottomColor = this.option.progressColor;
        }
    }
}
//条形进度条
class BarProgress {
    ...
    setProgressColor(){
        if (this.option.bgColor)
            this.container.querySelector('.bar').style.background = this.option.bgColor;
        if (this.option.progressColor)
            this.container.querySelector('.completed').style.background = this.option.progressColor;
    }
}
//圆环进度条
class RingProgress {
    ...
    getProgressHtml(){
        const html = `<svg xmlns="http://www.w3.org/200/svg" height="150" width="110">
                            ...
                            <circle transform="rotate(-90,55 55)" class="demo2" id="J_demo2" cx="55" cy="55" r="50" fill="none" stroke="red" stroke-width="10" stroke-dasharray="0,1000"/>
                        </svg>`;
        return html;
    }
    getProgressValue(){
    ...
    setProgressColor(){
        if (this.option.bgColor)
            this.container.querySelectorAll('circle')[0].setAttribute('stroke',this.option.bgColor);
        if (this.option.progressColor)
            this.container.querySelectorAll('circle')[1].setAttribute('stroke',this.option.progressColor);
    }
}
