<template lang="html">
  ...
  <div class="shopCart">
    <div class="content">
      ...
      <div @click="subOrder($event)" class="content-right" :class="{'enough':totalPrice>=minPrice}">
        {{payDesc}}
      </div>
    </div>
    ...
    <transition name="transHeight">
      <div class="shopcart-list" v-show="listShow">
        ...
        <div class="list-content" ref="foodlist">
          <ul class="orderAllfood">
            <li class="food" v-for="food in selectFoods">
              <span class="foodname name">{{food.foodname}}</span>
              <div class="price">
                <span>￥{{food.price * food.count}}</span>
              </div>
              <div class="food-info" :price="food.price" :count="food.count"></div>
...
export default {
  ...
  methods: {
    ...
    subOrder(ev) {
      /* 首先把字符串弄出来 */
      let allFoodLi = $('.orderAllfood li');
      let len = allFoodLi.length;
      let foodStr = '';
      let foodname = '';
      let foodCount = '';
      let foodPrice = '';
      let extraIcon = '';
      console.log('点的餐的长度length为', len);
      for (let i = 0; i < len; i++) {
        foodname = $(allFoodLi[i]).find('.foodname').text();
        foodCount = $(allFoodLi[i]).find('.food-info').attr('count');
        foodPrice = $(allFoodLi[i]).find('.food-info').attr('price');
        console.log('前面每一个的foodstr为', foodStr);
        foodStr = foodStr + extraIcon + foodname + ',' + foodCount + ',' + foodPrice;
        extraIcon = ';';
      }
      console.log('提交订单这里的字符串是', foodStr);
      $.post('orderGoodsServlet', {
        username: this.$store.state.user.username,
        foods: foodStr
      }, (res) => {
        console.log('购物车模块返回的消息', res);
        console.log('提交订单这里的字符串是', foodStr);
        let data = JSON.parse(res);
        console.log(this.$parent);
        alert('下单成功');
        this.$parent.reloadGoods();
      });
    }
  },
  components: {
