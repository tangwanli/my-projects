...
> reset样式是.css文件,直接引入到index.html里面去的，而不是用的.styl来写的reset

#### 1、main.js

>+ (1)**插件**：下面两个是再vue.js中引入的插件
>    + vueTap插件：实现vue中的touch事件模拟click，解决点击延迟问题
>    + fastclick库：解决移动端点透问题，即300毫秒点击延迟
...
>+ 页面结构：结构为**上面**为引入的一个**header模块; 中间**是一个**导航条(导航条里面是router-link用来点击); 下面**就是页面的**主体部分(即一个router-view)**
>    + 主体部分可以切换为3种 分别是**商品模块(goods)、评论模块(ratings)、商家模块(seller)**
>+ 这个的样式是用styuls写的，**引入的index.styuls里面就是3个基础样式``base.styl、minin.styl、ico.styl``**
>+ 这个的js里面有一个**seller**变量; 是通过**axios.get方法**获取到的,这个接收到的返回值就是一个**存用户信息的seller**，模拟用的就是**static文件夹里面的data.json**
