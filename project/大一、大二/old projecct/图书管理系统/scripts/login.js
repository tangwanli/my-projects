  ...
  function userValidate() {
    ...
    if((inputUsername.val() == null) == false) {
        var patten = new RegExp("\\w(?=@twl.hero)")
        if(patten.test(inputUsername.val()))
        {
        ...
        $.post("LoginServlet", {
            ...
            if(data.isReturn == "true") {
                $(".firstPoint").css("color","green").text("*兄台好久不见");
            } else {
                // 这里需要加一个else，避免输入了正确用户名之后，再把用户名删除了，还是现实绿色的提示
                $(".firstPoint").css("color","red").text("*道不同不相为谋");
            }
        },"json");
        }
    }  
  ...
  function passwordValidate() {
    ...
    if((inputPassword.val() == null || inputPassword.val() == "") == false) {
        $.post("LoginServlet", {
            ...
            console.log("验证用户名服务器返回的值为" + data.isReturn + " 类型为" +typeof(data.isReturn));
            if(data.isReturn == "true") {
                $(".secondPoint").css("color","green").text("*江湖凶险，保重");
            } else {
                $(".secondPoint").css("color","red").text("*竟想混入江湖");
            }
        },"json");
